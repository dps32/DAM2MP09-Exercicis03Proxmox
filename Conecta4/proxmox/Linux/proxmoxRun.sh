...
# Calculate directories robustly: script is in Conecta4/proxmox/Linux
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$(dirname "$SCRIPT_DIR")")"  # -> Conecta4
JAR_PATH="$PROJECT_ROOT/target/$JAR_NAME"

ORIGINAL_PWD="$(pwd)"

cd "$PROJECT_ROOT" || { echo "Error: no s'ha pogut entrar a $PROJECT_ROOT"; exit 1; }
...
rm -f "$JAR_PATH"
if [[ ! -x "$PROJECT_ROOT/run.sh" && ! -f "$PROJECT_ROOT/run.sh" ]]; then
  echo "Error: no s'ha trobat run.sh a $PROJECT_ROOT/run.sh"
  cd "$ORIGINAL_PWD"
  exit 1
fi

"$PROJECT_ROOT/run.sh" com.server.Main build
